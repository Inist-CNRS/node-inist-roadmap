...
module.exports = function getBoard(boardLink) {
  ...
  const r1 = trello.makeRequest('get', `/1/boards/${boardID}`);
  const r2 = trello.makeRequest('get', `/1/boards/${boardID}/lists`);
  const r3 = trello.makeRequest('get', `/1/boards/${boardID}/cards`, { attachments: true });
  ...
  return Promise.all([r1, r2, r3]).then(values => new Promise((resolve) => {
    ...
    const boardCards = cards.reduce(convertCards, []).map((card) => {
